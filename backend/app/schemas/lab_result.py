...
class ReferenceRange(BaseModel):
    ...
    type: Optional[str] = Field(default=None, exclude=True, description="Range type: TWO_SIDED, UPPER_ONLY, LOWER_ONLY, DEMOGRAPHIC, PREGNANCY, CATEGORICAL")
    operator: Optional[str] = Field(default=None, exclude=True, description="Range operator: <=, <, >=, >")
    selected_group: Optional[str] = Field(default=None, exclude=True, description="Selected demographic or pregnancy group")
    categories: Optional[List[Dict[str, Any]]] = Field(default=None, exclude=True, description="Categorical reference range definitions")
    selected_category: Optional[str] = Field(default=None, exclude=True, description="Selected qualitative category label")

class LabTestResult(BaseModel):
    raw_test_name: str = Field(..., description="Raw extracted laboratory test name")
    canonical_test_name: Optional[str] = Field(default=None, exclude=True, description="Canonical normalized test name (Phase 2)")
