...
@pytest.mark.asyncio
async def test_upload_pdf_report_success(async_client: AsyncClient, sample_pdf_bytes: bytes):
    files = {
        "file": ("blood_test_report.pdf", sample_pdf_bytes, "application/pdf")
    }
...
async def test_upload_image_report_success(async_client: AsyncClient, sample_image_bytes: bytes):
    files = {
        "file": ("blood_report_scanned.png", sample_image_bytes, "image/png")
    }
...
async def test_upload_invalid_file_extension(async_client: AsyncClient):
    files = {
        "file": ("unsupported_file.docx", b"dummy content", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    }
